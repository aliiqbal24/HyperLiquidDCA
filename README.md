...
- Paid schedules execute from cloud infrastructure so they can run 24/7.
...
- Vercel/Next cloud API with Neon Postgres storage, encrypted API-wallet credentials, cron execution, Stripe Checkout, Stripe Customer Portal, and billing webhooks.
...
2. Create Stripe subscription prices for Cloud Lite and Cloud Pro.
3. Set the API environment variables from `.env.example` in Vercel.
4. Deploy `apps/api` as the Vercel project.
5. Set `VITE_API_BASE_URL` and Stripe price IDs for the extension build.
